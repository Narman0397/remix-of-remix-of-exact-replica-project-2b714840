  ...
  return (
    <div className="space-y-3 rounded-xl border border-border bg-card p-4">
      ...
      <div>
        ...
        <textarea
          value={meta.deskripsi}
          onChange={(e) => setMeta({ ...meta, deskripsi: e.target.value })}
          disabled={readOnly}
          rows={3}
          className="mt-1 w-full rounded-md border border-border bg-background px-3 py-2 text-sm disabled:opacity-60"
        />
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <label className="flex items-center gap-2 text-sm self-end">
          <input
            type="checkbox"
            checked={meta.allow_multiple_submit}
            onChange={(e) => setMeta({ ...meta, allow_multiple_submit: e.target.checked })}
            disabled={readOnly}
          />
      ...
      {!readOnly && (
        ...
        >
          <Save className="h-4 w-4" /> Simpan Metadata
        </button>
      )}
    </div>
  );
