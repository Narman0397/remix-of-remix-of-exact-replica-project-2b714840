  ...
  head: () => ({
    meta: [
      { title: "Satu Data — Pemerintah Kabupaten Buton Selatan" },
      {
        name: "description",
        content:
          "Dashboard data terpadu Kabupaten Buton Selatan: penduduk, anggaran, kinerja layanan, dan ekonomi.",
      },
    ...
    async function load() {
      ...
      const [itemsRes, modeRes] = await Promise.all([
        supabase
          ...
          .select(
            "id,kategori,label,nilai_teks,nilai_num,nilai_num2,trend,ikon,format,ukuran,url,opd,urutan",
          )
          .eq("aktif", true)
  ...
  const layananBulanan = useMemo(
    () =>
      items
        ...
        .map((i) => ({
          bulan: i.label,
          permohonan: Number(i.nilai_num ?? 0),
          selesai: Number(i.nilai_num2 ?? 0),
        })),
    [items],
  );
        ...
        <div className="container-page py-20">
          <div className="mx-auto max-w-md rounded-xl border border-border bg-card p-8 text-center shadow-soft">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Halaman Satu Data hanya dapat diakses setelah Anda masuk.
            </p>
            <Link
              to="/auth"
      ...
      {items.length === 0 && !loadError && (
        <div className="container-page mt-6">
          <div className="rounded-lg border border-border bg-card p-6 text-center text-sm text-muted-foreground">
            Belum ada item data terpadu yang aktif. Super admin dapat menambahkan via menu{" "}
      ...
      {accessMode === "auth" && isSuperAdmin && (
        <div className="container-page mt-6">
          <div className="rounded-lg border border-gold/40 bg-gold/15 p-3 text-xs text-gold-foreground">
            Halaman ini diatur <strong>perlu login</strong>. Pengunjung anonim akan diarahkan ke
            halaman masuk.
          </div>
        </div>
      )}
