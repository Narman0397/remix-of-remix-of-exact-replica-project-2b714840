...
function Page() {
  ...
  const m = useMutation({
    mutationFn: rec,
    onSuccess: () => {
      toast.success("Hasil dicatat");
      qc.invalidateQueries({ queryKey: ["uat"] });
    },
  ...
  function setStatus(scenario_id: string, status: "pass" | "partial" | "fail") {
    const catatan =
      prompt(`Catatan untuk hasil ${status.toUpperCase()} (opsional):`, "") ?? undefined;
  ...
  return (
    <AdminShell breadcrumb={[{ label: "System" }, { label: "UAT" }]}>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Skenario UAT per role. Catat hasil setelah verifikasi manual.
      </p>
              ...
              return (
                <tr key={r.id} className="border-t border-border">
                  ...
                  <td className="px-3 py-2">
                    <div className="text-sm">{r.judul}</div>
                    {r.expected && (
                      <div className="text-xs text-muted-foreground">Expected: {r.expected}</div>
                    )}
                  </td>
                  <td className="px-3 py-2">
                    {st === "pass" && (
                    ...
                    {st === "partial" && (
                      <span className="rounded bg-gold/20 px-2 py-0.5 text-xs">PARTIAL</span>
                    )}
                    {st === "fail" && (
                    ...
                    {r.last && (
                      <div className="mt-1 text-[10px] text-muted-foreground">
                        {new Date(r.last.run_at).toLocaleString("id-ID")}
                      </div>
                    )}
                  </td>
                  <td className="px-3 py-2">
                    <div className="flex gap-1">
                      <button
                        ...
                        className="rounded border border-border bg-card px-2 py-1 text-xs hover:bg-success/10"
                      >
                        ✓
                      </button>
                      <button
                        onClick={() => setStatus(r.id, "partial")}
                        className="rounded border border-border bg-card px-2 py-1 text-xs hover:bg-gold/10"
                      >
                        ~
                      </button>
                      <button
                        onClick={() => setStatus(r.id, "fail")}
                        className="rounded border border-border bg-card px-2 py-1 text-xs hover:bg-destructive/10"
                      >
                        ✕
                      </button>
                    </div>
