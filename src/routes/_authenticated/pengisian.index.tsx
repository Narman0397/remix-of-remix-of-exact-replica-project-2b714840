    ...
    <PageShell>
      <section className="container-page py-8">
        <div className="mb-4 rounded-xl border border-amber-300/50 bg-amber-50 px-4 py-3 text-xs text-amber-900 dark:bg-amber-950/30 dark:text-amber-200">
          <strong>Modul lama.</strong> Tugas pengisian terbaru ada di{" "}
          <Link to="/asn/tugas" className="font-semibold underline">
            Tugas Saya
          </Link>{" "}
          dengan draft, autosave, dan upload bukti.
        </div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Daftar formulir/laporan yang perlu Anda isi sebagai ASN.
        </p>
        ...
        <div className="mt-6 grid gap-3 md:grid-cols-2">
          ...
          {!busy && rows.length === 0 && (
            <div className="col-span-full rounded-xl border border-dashed border-border p-8 text-center text-sm text-muted-foreground">
              Tidak ada dataset yang ditujukan untuk Anda.
            </div>
          )}
          {rows.map((t) => {
            ...
            return (
              <Link
                ...
                to="/pengisian/$id"
                params={{ id: t.id }}
                className="rounded-xl border border-border bg-card p-4 transition hover:border-primary/40 hover:shadow-soft"
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  {t.deadline && (
                    <span
                      className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${overdue ? "bg-destructive/15 text-destructive" : "bg-warning/15 text-warning"}`}
                    >
