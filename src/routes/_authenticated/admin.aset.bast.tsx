...
type BastRow = {
  ...
  status: string;
  catatan: string | null;
  pemberi: { nama_lengkap: string } | null;
  penerima: { nama_lengkap: string } | null;
...
function Page() {
  ...
  const onApprove = async (id: string) => {
    try {
      await approveBast({ data: { id } });
      toast.success("BAST disetujui");
      void load();
    } catch (e) {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between">
        <div>
          <h2 className="font-display text-xl font-bold flex items-center gap-2">
            <FileSignature className="h-5 w-5" /> BAST Aset
          </h2>
          <p className="text-sm text-muted-foreground">
            Berita Acara Serah Terima aset. Setelah disetujui penerima, pemegang aset otomatis
            berpindah.
          </p>
        </div>
        <Button size="sm" onClick={() => setShowForm((v) => !v)}>
          <Plus className="h-3.5 w-3.5 mr-1" /> Buat BAST
        </Button>
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Daftar BAST ({rows.length})</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left border-b">
                  ...
                  <th className="p-2">Pemberi → Penerima</th>
              ...
              <tbody>
                {rows.map((r) => (
                  <tr key={r.id} className="border-b">
                    ...
                    <td className="p-2">
                      {r.pemberi?.nama_lengkap ?? "-"} → {r.penerima?.nama_lengkap ?? "-"}
                    </td>
...
function FormBast({ onCreated }: { onCreated: () => void }) {
  ...
  useEffect(() => {
    ...
    void listAset({ data: {} }).then((r) =>
      setAsets(r.rows as unknown as Array<{ id: string; kode: string; nama: string }>),
    );
  }, []);
  ...
  const submit = async () => {
    ...
    try {
      const r = await createBast({
        data: { penerima_user: penerima, aset_ids: Array.from(selected), catatan: catatan || null },
      });
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-sm">Form BAST Baru</CardTitle>
      </CardHeader>
        ...
        <div>
          ...
          <div className="max-h-64 overflow-y-auto border rounded-md">
            {asets.map((a) => (
              <label
                key={a.id}
                className="flex items-center gap-2 p-2 border-b text-sm hover:bg-muted/30"
              >
                <input
                  type="checkbox"
