...
function Page() {
  const [data, setData] = useState<{ gedungs: Gedung[]; lantais: Lantai[]; ruangans: Ruangan[] }>({
    gedungs: [],
    lantais: [],
    ruangans: [],
  });
  ...
  const addG = async () => {
    if (!nameG) return;
    try {
      await upsertGedung({ data: { nama: nameG, alamat: alamat || null, opd_id: null } });
      setNameG("");
      setAlamat("");
      void load();
    } catch (e) {
  ...
  const addL = async () => {
    ...
    try {
      await upsertLantai({ data: { gedung_id: gedungId, nama: nameL, urutan: 0 } });
      setNameL("");
      void load();
    } catch (e) {
      toast.error((e as Error).message);
  ...
  const addR = async () => {
    ...
    try {
      await upsertRuangan({ data: { lantai_id: lantaiId, nama: nameR } });
      setNameR("");
      void load();
    } catch (e) {
      toast.error((e as Error).message);
    ...
    <div className="space-y-4">
      <div>
        <h2 className="font-display text-xl font-bold flex items-center gap-2">
          <Building2 className="h-5 w-5" /> Lokasi (Gedung → Lantai → Ruangan)
        </h2>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Gedung</CardTitle>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Lantai</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            <select
            ...
            <div className="space-y-1 mt-2 max-h-64 overflow-y-auto">
              {data.lantais
                .filter((l) => !gedungId || l.gedung_id === gedungId)
                .map((l) => (
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Ruangan</CardTitle>
          </CardHeader>
          <CardContent className="space-y-2">
            <select
              className="w-full h-9 rounded-md border bg-background px-2 text-sm"
            ...
            />
            <Button size="sm" onClick={addR}>
              <Plus className="h-3.5 w-3.5 mr-1" /> Tambah
            </Button>
            <div className="space-y-1 mt-2 max-h-64 overflow-y-auto">
              {data.ruangans
                .filter((r) => !lantaiId || r.lantai_id === lantaiId)
                .map((r) => (
                  <div key={r.id} className="text-sm p-2 border rounded">
                    {r.nama}{" "}
                    {r.kode && <span className="text-xs text-muted-foreground">({r.kode})</span>}
                  </div>
                ))}
            </div>
          </CardContent>
