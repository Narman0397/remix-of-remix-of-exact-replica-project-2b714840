    ...
    <AdminShell breadcrumb={[{ label: "CMS" }]}>
      <div className="mb-4">
        <h1 className="font-display text-2xl font-bold flex items-center gap-2">
          <FileText className="h-6 w-6 text-primary" /> Manajemen Berita
        </h1>
        <p className="text-sm text-muted-foreground">
          Editor untuk konten halaman publik /berita. Untuk layanan OPD gunakan menu{" "}
          <strong>Layanan OPD</strong>.
        </p>
      </div>
      <BeritaTab />
    </AdminShell>
...
function BeritaTab() {
  ...
  async function load() {
    ...
    const { data } = await supabase
      .from("berita")
      .select("id,judul,ringkasan,isi,gambar_url,status,published_at")
    ...
    try {
      await upsertBerita({
        data: {
          id: editing.id,
          judul: editing.judul,
          ringkasan: editing.ringkasan ?? null,
          isi: editing.isi ?? "",
          gambar_url: editing.gambar_url ?? null,
          status: (editing.status ?? "draft") as "draft" | "terbit",
        },
      });
            ...
            {rows.map((b) => (
              <tr key={b.id} className="border-t border-border">
                <td className="px-4 py-3">
                  ...
                  {b.ringkasan && (
                    <div className="line-clamp-1 text-xs text-muted-foreground">{b.ringkasan}</div>
                  )}
                </td>
                <td className="px-4 py-3">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${b.status === "terbit" ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}
                  >
                    {b.status}
                  </span>
                </td>
          ...
          <div className="w-full max-w-2xl rounded-xl border border-border bg-card p-6 shadow-elevated max-h-[90vh] overflow-y-auto">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">
                {editing.id ? "Edit Berita" : "Berita Baru"}
              </h2>
            ...
            <div className="space-y-3">
              ...
              <Field label="Ringkasan">
                <input
                  ...
                  onChange={(e) => setEditing({ ...editing, ringkasan: e.target.value })}
                  className="mt-1 h-9 w-full rounded-md border border-border bg-background px-3 text-sm"
                />
              </Field>
              <Field label="Isi (mendukung paragraf)">
                <textarea
                  rows={8}
                  value={editing.isi ?? ""}
              ...
              <Field label="URL Gambar Sampul">
                <input
                  value={editing.gambar_url ?? ""}
                  onChange={(e) => setEditing({ ...editing, gambar_url: e.target.value })}
                  placeholder="https://..."
                  className="mt-1 h-9 w-full rounded-md border border-border bg-background px-3 text-sm"
                />
              </Field>
              <Field label="Status">
                <select
                  ...
                  onChange={(e) =>
                    setEditing({ ...editing, status: e.target.value as "draft" | "terbit" })
                  }
                  className="mt-1 h-9 w-full rounded-md border border-border bg-background px-3 text-sm"
                >
