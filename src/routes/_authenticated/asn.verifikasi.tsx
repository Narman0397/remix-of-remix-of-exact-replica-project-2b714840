...
import {
  listCampaigns,
  listCampaignItems,
  submitCampaignVerification,
} from "@/lib/aset-advanced.functions";

export const Route = createFileRoute("/_authenticated/asn/verifikasi")({
  head: () => ({
    meta: [{ title: "Verifikasi Aset — ASN" }, { name: "robots", content: "noindex" }],
  }),
  component: Page,
});
...
function Page() {
  const { user, isAsn, isVerified, loading, refreshProfile } = useAuth();
  useEffect(() => {
    ...
    const id = navigator.geolocation.watchPosition(
      (p) => {
        ...
        setCoords((prev) => {
          if (
            prev &&
            Math.abs(prev.lat - p.coords.latitude) < 1e-5 &&
            Math.abs(prev.lng - p.coords.longitude) < 1e-5
          )
            return prev;
      ...
      (err) =>
        setGpsError(err.code === 1 ? "Izin lokasi ditolak." : "Gagal mendapatkan lokasi GPS."),
  ...
  useEffect(() => {
    if (campId) reload(campId, filter);
  }, [campId, filter, reload]);
  ...
  function onPickPhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (ageSec > 600 && !cameraName) {
      toast.error("Foto harus langsung dari kamera.");
      return;
    }
    setPhotoBlob(file);
  ...
  async function submit() {
    ...
    if (!coords) {
      toast.error("GPS wajib aktif.");
      return;
    }
    if (!photoBlob) {
      toast.error("Foto fisik aset wajib.");
      return;
    }
    setBusy(true);
    try {
      ...
      const { error: upErr } = await supabase.storage
        .from("aset-foto")
        .upload(path, photoBlob, { contentType: photoBlob.type, upsert: true });
      if (upErr) throw new Error(`Gagal unggah foto: ${upErr.message}`);
      await submitCampaignVerification({
        data: {
          item_id: selected.id,
          lat: coords.lat,
          lng: coords.lng,
  ...
  if (loading)
    return (
      <PageShell>
        <div className="container-page py-10">Memuat…</div>
      </PageShell>
    );
  if (!user)
    return (
  ...
  if (!isAsn)
    return (
      <PageShell>
        <div className="container-page py-10">Halaman ini khusus ASN.</div>
      </PageShell>
    );
  if (!isVerified)
    return (
      <PageShell>
        <div className="container-page py-10">Akun ASN Anda belum diverifikasi.</div>
      </PageShell>
    );
    ...
    <PageShell>
      <section className="container-page py-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Pilih kampanye aktif, lalu verifikasi tiap aset dengan foto fisik & lokasi GPS.
        </p>
          ...
          >
            ...
            {camps.map((c) => (
              <option key={c.id} value={c.id}>
                {c.nama} ({c.periode_mulai}…{c.periode_selesai})
              </option>
            ))}
          </select>
          <div className="inline-flex rounded-lg border border-border bg-surface p-1">
            {(["belum", "selesai"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setFilter(t)}
                className={`h-8 px-3 rounded-md text-xs font-semibold ${filter === t ? "bg-gradient-primary text-primary-foreground" : "text-muted-foreground"}`}
              >
        ...
        <div className="mt-4 grid gap-6 md:grid-cols-2">
          <div className="space-y-2">
            ...
            <div className="rounded-xl border border-border bg-card divide-y divide-border max-h-[520px] overflow-y-auto">
              {items.length === 0 && (
                <div className="p-6 text-center text-sm text-muted-foreground">Tidak ada item.</div>
              )}
              {items.map((it) => (
                <button
                  key={it.id}
                  onClick={() => setSelected(it)}
                  className={`block w-full text-left px-3 py-3 hover:bg-muted/50 ${selected?.id === it.id ? "bg-primary/5" : ""}`}
                >
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="font-semibold text-sm">{it.aset?.nama ?? "—"}</div>
                      <div className="text-xs text-muted-foreground">
                        {it.aset?.kode} · {it.aset?.kategori} · {it.opd?.singkatan}
                      </div>
                    </div>
                    {it.status === "selesai" ? (
            ...
            {!selected ? (
              ...
              <div className="rounded-xl border border-border bg-card p-4 space-y-3">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {selected.aset?.kode} · {selected.aset?.kategori}
                  </div>
                </div>

                {selected.status === "selesai" ? (
                  <div className="rounded-md bg-success/10 p-3 text-sm text-success">
                    Aset ini sudah diverifikasi pada{" "}
                    {new Date(selected.verified_at!).toLocaleString("id-ID")}.
                  </div>
                ) : (
                  <>
                    <div>
                      <label className="text-xs font-medium text-muted-foreground">
                        Foto fisik aset (wajib)
                      </label>
                      <input
                        ref={cameraInputRef}
                        type="file"
                      ...
                      <button
                        ...
                        className="mt-1 inline-flex h-10 items-center gap-2 rounded-md border border-border bg-background px-3 text-sm"
                      >
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder="Mis. kondisi baik, ada lecet kecil di sisi kanan…"
                      />
                    </div>
                    ...
                    <div className="flex gap-2">
                      <button
                        disabled={busy || !coords || !photoBlob}
                        onClick={submit}
                        className="h-10 rounded-md bg-gradient-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-60"
                      >
