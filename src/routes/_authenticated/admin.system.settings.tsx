...
export const Route = createFileRoute("/_authenticated/admin/system/settings")({
  head: () => ({
    meta: [{ title: "Settings Governance — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <SuperAdminOnly>
  ...
  return (
    <AdminShell breadcrumb={[{ label: "System" }, { label: "Settings" }]}>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Audit dan kategori setting aplikasi. Setting sensitif tidak boleh `public`.
      </p>
            ...
            {(data as Row[] | undefined)?.map((r) => (
              <tr key={r.key} className="border-t border-border">
                ...
                <td className="px-4 py-3">
                  <pre className="max-w-md overflow-x-auto text-[10px]">
