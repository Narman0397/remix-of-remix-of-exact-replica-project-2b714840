...
export const Route = createFileRoute("/_authenticated/admin/asn-kepatuhan")({
  head: () => ({
    meta: [{ title: "Kepatuhan Kehadiran ASN" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <Page />
...
function JadwalEditor({ list, onChange }: { list: Sch[]; onChange: () => void }) {
  ...
  async function save() {
    try {
      await upsertSchedule({ data: { ...form, opd_id: null, aktif: true } });
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-4">
          ...
          <input
            ...
            onChange={(e) => setForm({ ...form, toleransi_menit: parseInt(e.target.value) || 0 })}
          />
        </div>
        <button
          onClick={save}
          className="mt-3 h-10 rounded-md bg-gradient-primary px-4 text-sm font-semibold text-primary-foreground"
        >
          Simpan
        </button>
      </div>
      <div className="rounded-xl border border-border bg-card">
        {list.length === 0 && (
          <div className="p-6 text-center text-sm text-muted-foreground">Belum ada jadwal.</div>
        )}
        {list.map((s) => (
          <div
            key={s.id}
...
function ShiftEditor({ list, onChange }: { list: Sh[]; onChange: () => void }) {
  ...
  async function save() {
    try {
      await upsertShift({ data: { ...form, opd_id: null, aktif: true } });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card">
        {list.length === 0 && (
          <div className="p-6 text-center text-sm text-muted-foreground">Belum ada shift.</div>
        )}
        {list.map((s) => (
          <div
            key={s.id}
          ...
          >
            <div>
              <div className="font-semibold">
                {s.nama} <span className="text-xs text-muted-foreground">({s.jenis})</span>
              </div>
              <div className="text-xs text-muted-foreground">
                {s.jam_masuk}–{s.jam_pulang} · toleransi {s.toleransi_menit} mnt
              </div>
            ...
            <button
              onClick={async () => {
                await deleteShift({ data: { id: s.id } });
                onChange();
              }}
              className="text-xs text-destructive"
            >
