...
export const Route = createFileRoute("/_authenticated/admin/layanan/escalation")({
  head: () => ({
    meta: [{ title: "Escalation SLA — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <SuperAdminOnly>
...
function Page() {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Threshold per Level</CardTitle>
      </CardHeader>
      <CardContent className="space-y-3">
        <p className="text-sm text-muted-foreground">
          Konfigurasi global (OPD = kosong). Default: L1=1 hari, L2=3 hari, L3=7 hari. Cron{" "}
          <code>sla-escalation</code> berjalan setiap 30 menit dan memerlukan flag{" "}
          <code>escalation.enabled</code> aktif.
        </p>
            ...
            <div key={r.id} className="flex flex-wrap items-center gap-3 rounded-md border p-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setRows((arr) =>
                      arr.map((x, j) =>
                        j === i ? { ...x, threshold_days: Number(e.target.value) } : x,
                      ),
                    )
                  }
                />
              ...
              <div className="flex items-center gap-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setRows((arr) =>
                      arr.map((x, j) => (j === i ? { ...x, target_role: e.target.value } : x)),
                    )
                  }
                />
              </div>
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(e) =>
                    setRows((arr) =>
                      arr.map((x, j) => (j === i ? { ...x, aktif: e.target.checked } : x)),
                    )
                  }
                />
                Aktif
