...
function Page() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setTpl(t.template);
      const my = (await mySubmission({ data: { template_id: id } })) as unknown as {
        rows: Submission[];
      };
      if (my.rows[0]) {
  ...
  if (!user)
    return (
      <PageShell>
        <div className="container-page py-10">Silakan masuk.</div>
      </PageShell>
    );
  if (!tpl)
    return (
      <PageShell>
        <div className="container-page py-10">{err ?? "Memuat…"}</div>
      </PageShell>
    );
      ...
      <section className="container-page py-8">
        ...
        <div className="mt-3 flex items-start justify-between gap-3">
          <div>
            ...
            {tpl.deskripsi && (
              <p className="mt-1 max-w-2xl text-sm text-muted-foreground">{tpl.deskripsi}</p>
            )}
              ...
              {tpl.deadline && (
                <>
                  ...
                  <span className={overdue ? "text-destructive font-semibold" : ""}>
                    {new Date(tpl.deadline).toLocaleString("id-ID")}
                  </span>
                </>
              )}
            </div>
          ...
          {existing && (
            <span className="rounded-full bg-success/15 px-3 py-1 text-xs font-semibold uppercase text-success">
              Sudah Disubmit
            </span>
          )}
        </div>
        ...
        <div className="mt-6 max-w-3xl space-y-4 rounded-xl border border-border bg-card p-6">
          {tpl.kolom.map((k) => (
            <div key={k.key}>
              ...
              {k.tipe === "textarea" ? (
                ...
                <input
                  className="input"
                  type={k.tipe === "number" ? "number" : k.tipe === "date" ? "date" : "text"}
          ...
          {ok && (
            <div className="inline-flex items-center gap-1 rounded-md border border-success/40 bg-success/10 p-2 text-xs text-success">
          ...
          <button
            onClick={onSubmit}
            disabled={busy || !tpl.aktif || !!overdue}
          ...
          >
            ...
            {busy ? "Menyimpan…" : existing ? "Perbarui Submission" : "Kirim Submission"}
          </button>
        </div>
      </section>
    </PageShell>
