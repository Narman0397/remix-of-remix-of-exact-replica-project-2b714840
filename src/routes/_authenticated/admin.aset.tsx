  ...
  return (
    <AdminShell breadcrumb={[{ label: "Modul Aset" }]}>
      ...
      <p className="text-sm text-muted-foreground">
        Kelola data aset, cetak QR untuk pelacakan, dan pantau riwayat scan lapangan.
      </p>
...
type AsetRow = {
  ...
  pemegang: { nama_lengkap: string; nip: string | null } | null;
};
...
function AsetTab({ opds }: { opds: Opd[] }) {
  ...
  const [form, setForm] = useState<{
    nama: string;
    kategori: "kendaraan" | "elektronik" | "lainnya";
    merk: string;
    nomor_seri: string;
    opd_id: string;
  }>({ nama: "", kategori: "kendaraan", merk: "", nomor_seri: "", opd_id: "" });
  ...
  async function reload() {
    const r = await listAset({ data: {} });
    setRows((r as unknown as { rows: AsetRow[] }).rows);
  }
  useEffect(() => {
    reload().catch(() => {});
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (t && !`${r.nama} ${r.kode} ${r.opd?.singkatan ?? ""}`.toLowerCase().includes(t))
        return false;
      return true;
    });
  ...
  async function create() {
    if (!form.nama || !form.opd_id) {
      toast.error("Nama & OPD wajib");
      return;
    }
    setBusy(true);
    try {
      ...
      setForm({ nama: "", kategori: "kendaraan", merk: "", nomor_seri: "", opd_id: "" });
      await reload();
    } catch (e) {
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        ...
        <button
          disabled={busy}
          onClick={create}
          className="mt-2 inline-flex h-9 items-center gap-1 rounded-md bg-gradient-primary px-3 text-xs font-semibold text-primary-foreground"
        >
      ...
      <div className="flex flex-wrap items-center gap-2">
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder="Cari nama / kode / OPD"
              ...
              return (
                <tr key={r.id} className="border-t border-border align-top">
                  ...
                  <td className="px-3 py-2 whitespace-nowrap">
                    ...
                    >
                      <QrCode className="h-3 w-3" /> Pratinjau
                    </button>
                    <button
                      onClick={() => downloadQrPng(url, `aset-${r.kode}.png`)}
                      className="ml-1 inline-flex h-8 items-center gap-1 rounded-md border border-border px-2 text-xs"
                    >
                      Unduh
                    </button>
                  </td>
                  <td className="px-3 py-2">
                    <button
                      onClick={() => hapus(r.id)}
                      className="inline-flex h-8 items-center gap-1 rounded-md border border-destructive/40 px-2 text-xs text-destructive"
                    >
      ...
      {qrFor && (
        ...
        >
          <div
            className="rounded-xl bg-card p-6 text-center shadow-elevated"
          ...
          >
            ...
            <div className="mt-3 grid place-items-center">
              <QrImage value={`${window.location.origin}/asn/aset?kode=${qrFor.kode}`} size={240} />
            </div>
            <button
              onClick={() =>
                downloadQrPng(
                  `${window.location.origin}/asn/aset?kode=${qrFor.kode}`,
                  `aset-${qrFor.kode}.png`,
                )
              }
              className="mt-3 h-9 rounded-md bg-gradient-primary px-3 text-xs font-semibold text-primary-foreground"
            >
              Unduh PNG
            </button>
          </div>
...
type Riwayat = {
  ...
  lat: number | null;
  lng: number | null;
  data: { foto_url?: string } | null;
  created_at: string;
  oleh_profile: { nama_lengkap: string } | null;
};
...
function RiwayatTab() {
  ...
  useEffect(() => {
    listAset({ data: {} })
      .then((r) =>
        setList(
          (r as { rows: { id: string; kode: string; nama: string }[] }).rows.map((x) => ({
            id: x.id,
            kode: x.kode,
            nama: x.nama,
          })),
  ...
  async function load() {
    if (!asetId) return;
    const r = await listAsetRiwayat({ data: { aset_id: asetId } });
    setRows((r as unknown as { rows: Riwayat[] }).rows);
  }
  useEffect(() => {
    load().catch(() => {});
  }, [asetId]);
  ...
  return (
    <div className="space-y-3">
      ...
      {selected && (
        <div className="text-xs text-muted-foreground">
          Riwayat aset: <b>{selected.nama}</b>
        </div>
      )}
      <div className="rounded-xl border border-border bg-card divide-y divide-border">
        {rows.length === 0 && (
          <div className="p-6 text-center text-sm text-muted-foreground">
            Pilih aset untuk melihat riwayat.
          </div>
        )}
        {rows.map((r) => (
          <div key={r.id} className="p-3 text-sm">
            <div className="flex items-center justify-between">
              <span className="font-semibold">{r.aksi}</span>
              <span className="text-xs text-muted-foreground">
                {new Date(r.created_at).toLocaleString("id-ID")}
              </span>
            </div>
            <div className="text-xs text-muted-foreground">
              Oleh: {r.oleh_profile?.nama_lengkap ?? "-"}
            </div>
