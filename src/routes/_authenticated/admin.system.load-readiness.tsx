...
export const Route = createFileRoute("/_authenticated/admin/system/load-readiness")({
  head: () => ({
    meta: [{ title: "Load Readiness — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <SuperAdminOnly>
  ...
  const checklist = [
    {
      ...
      items: [
        ...
        { k: "Realtime backoff", ok: true },
        {
          k: "Cron success >95%",
          ok: stats
            ? stats.cronTotal === 0 || (stats.cronTotal - stats.cronFail) / stats.cronTotal >= 0.95
            : true,
        },
      ],
    },
    {
      tier: "500 concurrent users",
      items: [
  ...
  return (
    <AdminShell breadcrumb={[{ label: "System" }, { label: "Load Readiness" }]}>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Indikator kesiapan menerima beban tinggi.
      </p>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-4">
        <StatCard
          ...
          value={stats?.pending ?? "—"}
          icon={Server}
          tone={stats && stats.pending > 100 ? "destructive" : "default"}
        />
        <StatCard label="Job Running" value={stats?.running ?? "—"} icon={Activity} />
        <StatCard
          label="Cron Gagal 24j"
          value={stats ? `${stats.cronFail}/${stats.cronTotal}` : "—"}
          icon={Activity}
        />
            ...
            <ul className="divide-y divide-border">
              {tier.items.map((it) => (
                <li key={it.k} className="flex items-center justify-between px-4 py-2 text-sm">
                  <span>{it.k}</span>
                  <span
                    className={`rounded px-2 py-0.5 text-xs font-semibold ${it.ok ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}
                  >
                    {it.ok ? "PASS" : "FAIL"}
                  </span>
                </li>
