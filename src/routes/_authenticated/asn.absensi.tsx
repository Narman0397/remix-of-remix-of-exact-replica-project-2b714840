...
export const Route = createFileRoute("/_authenticated/asn/absensi")({
  head: () => ({
    meta: [{ title: "Absensi ASN — Scan QR" }, { name: "robots", content: "noindex" }],
  }),
...
function AbsensiPage() {
  ...
  // Backstop realtime: pastikan status verifikasi terbaru saat halaman dibuka.
  useEffect(() => {
  ...
  const requestGps = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setGpsError(
          err.code === 1
            ? "Izin lokasi ditolak. Aktifkan GPS pada peramban Anda untuk absen."
  ...
  useEffect(() => {
    ...
    if (t) {
      setScanned(t);
      sessionStorage.removeItem("kantor_qr_token");
    }
  }, []);
  ...
  const handleScan = useCallback((text: string) => {
    ...
    try {
      if (text.startsWith("http")) {
        ...
        const m = u.pathname.match(/\/asn\/scan\/([\w-]+)/);
        if (m) token = m[1];
    ...
    if (token) setScanned(token);
    else toast.error("QR tidak dikenali");
  }, []);
  ...
  if (!isAsn)
    return (
      <PageShell>
        <div className="container-page py-10">
          Halaman ini hanya untuk ASN terdaftar dan terverifikasi.
        </div>
      </PageShell>
    );
  if (!isVerified)
    return (
      <PageShell>
        <div className="container-page py-10">Akun ASN Anda belum diverifikasi Super Admin.</div>
      </PageShell>
    );
      ...
      <section className="container-page py-8">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Pilih tipe absen lalu scan QR. Foto wajah & GPS wajib aktif untuk mencegah titip
              absen.
            </p>
          </div>
          <Link
            to="/asn/izin"
            className="h-9 inline-flex items-center rounded-md border border-border bg-card px-3 text-xs font-semibold"
          >
            Ajukan Izin / Cuti
          </Link>
        </div>
        ...
        {schedule &&
          (() => {
            ...
            const masuk = new Date(now);
            masuk.setHours(hh, mm, 0, 0);
            const batas = new Date(masuk.getTime() + (schedule.toleransi_menit || 0) * 60000);
            const lateNow = tipe === "masuk" && now > batas;
            return (
              <div
                className={`mt-4 rounded-lg border p-3 text-sm ${lateNow ? "border-warning/50 bg-warning/10" : "border-border bg-surface"}`}
              >
                <div className="font-semibold">Jadwal hari ini: {schedule.nama}</div>
                <div className="text-xs text-muted-foreground">
                  Masuk {schedule.jam_masuk} · Pulang {schedule.jam_pulang} · Toleransi{" "}
                  {schedule.toleransi_menit} menit
                </div>
                {lateNow && (
                  <div className="mt-1 text-xs font-semibold text-warning">
                    ⚠ Anda sudah melewati batas keterlambatan (
                    {batas.toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })}).
                    Absen tetap dapat dikirim dan akan ditandai terlambat.
                  </div>
                )}
              </div>
            );
        ...
        {!gpsReady && (
          <div className="mt-4 rounded-lg border border-destructive/40 bg-destructive/10 p-3 text-sm">
            ...
            <button
              ...
              className="mt-2 h-9 rounded-md bg-gradient-primary px-3 text-xs font-semibold text-primary-foreground disabled:opacity-60"
            >
            ...
            {!gpsReady ? (
              ...
              <div className="rounded-xl border border-border bg-card p-4">
                <div className="text-sm">
                  Token terdeteksi. Konfirmasi absen <b>{tipe}</b>?
                </div>
            ...
            <div className="mt-2 max-h-[420px] overflow-y-auto rounded-xl border border-border bg-card">
              ...
              {rows.map((r) => (
                ...
                >
                  <div>
                    <span
                      className={`inline-block rounded px-1.5 py-0.5 text-[10px] font-semibold ${r.tipe === "masuk" ? "bg-success/15 text-success" : "bg-accent/20 text-accent"}`}
                    >
                      {r.tipe.toUpperCase()}
                    </span>
