...
import {
  ...
  listOpnameItems,
  verifyOpnameItem,
} from "@/lib/aset-opname.functions";
...
export const Route = createFileRoute("/_authenticated/admin/aset/opname")({
  head: () => ({
    meta: [{ title: "Admin — Opname Aset" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <AdminShell>
...
type ItemRow = {
  ...
  aset: { id: string; kode: string; nama: string } | null;
};
...
function Page() {
  ...
  const create = async () => {
    try {
      const r = await openOpname({ data: { periode } });
      toast.success(`Opname dibuka (${r.items} item)`);
      void load();
    } catch (e) {
  ...
  const close = async (id: string) => {
    ...
    try {
      await closeOpname({ data: { id } });
      toast.success("Ditutup");
      void load();
    } catch (e) {
      toast.error((e as Error).message);
    }
  ...
  const verify = async (
    ...
    try {
      await verifyOpnameItem({ data: { item_id, hadir, kondisi_temuan, catatan } });
    ...
    <div className="space-y-4">
      <div>
        <h2 className="font-display text-xl font-bold flex items-center gap-2">
          <ClipboardCheck className="h-5 w-5" /> Opname Aset
        </h2>
        <p className="text-sm text-muted-foreground">
          Inventarisasi fisik per periode. Snapshot otomatis dibuat saat opname dibuka.
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Buka Opname Baru</CardTitle>
        </CardHeader>
        <CardContent className="flex items-end gap-2">
          <div>
            <label className="text-xs text-muted-foreground">Periode</label>
            <Input value={periode} onChange={(e) => setPeriode(e.target.value)} className="w-40" />
          </div>
          <Button size="sm" onClick={create}>
            <Plus className="h-3.5 w-3.5 mr-1" /> Buka
          </Button>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-sm">Daftar Opname</CardTitle>
        </CardHeader>
        <CardContent>
              ...
              {rows.map((r) => (
                <tr key={r.id} className={`border-b ${activeId === r.id ? "bg-muted/30" : ""}`}>
                  ...
                  <td className="p-2 text-right space-x-1">
                    <Button size="sm" variant="outline" onClick={() => loadItems(r.id)}>
                      Buka
                    </Button>
                    {r.status === "open" && (
                      <Button size="sm" variant="ghost" onClick={() => close(r.id)}>
      ...
      {activeId && (
        <Card>
          <CardHeader>
            <CardTitle className="text-sm">Item Opname ({items.length})</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="text-left border-b">
                    ...
                    <th className="p-2 text-right">Verifikasi</th>
                  </tr>
                </thead>
                <tbody>
                  {items.map((it) => (
                    <tr key={it.id} className="border-b">
                      ...
                      <td className="p-2 text-right space-x-1">
                        <Button
                          size="sm"
                          variant="outline"
                          onClick={() => verify(it.id, true, "baik")}
                        >
                        ...
                        <Button
                          size="sm"
                          variant="ghost"
                          onClick={() => verify(it.id, false, "tidak_ditemukan")}
                        >
