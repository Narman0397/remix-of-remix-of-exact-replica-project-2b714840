...
export const Route = createFileRoute("/_authenticated/admin/system/disaster-recovery")({
  head: () => ({
    meta: [{ title: "Disaster Recovery — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <SuperAdminOnly>
...
const PROCS = [
  ...
  },
  {
    ...
    rto: "15 menit",
    rpo: "tidak ada (best-effort delivery)",
    steps: [
  ...
  return (
    <AdminShell breadcrumb={[{ label: "System" }, { label: "Disaster Recovery" }]}>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Prosedur pemulihan per skenario kegagalan.
      </p>
          ...
          <div key={p.title} className="rounded-xl border border-border bg-card p-4 shadow-soft">
            <div className="flex flex-wrap items-baseline justify-between gap-2">
              ...
              <div className="text-xs text-muted-foreground">
                RTO: <strong>{p.rto}</strong> · RPO: <strong>{p.rpo}</strong>
              </div>
            </div>
