...
export const Route = createFileRoute("/_authenticated/admin/asn/payroll-lock")({
  head: () => ({
    meta: [{ title: "Kunci Payroll — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <AdminShell breadcrumb={[{ label: "Admin" }, { label: "Payroll Lock" }]}>
        <Page />
      </AdminShell>
    </AdminGuard>
  ),
...
function Page() {
  ...
  return (
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <CardTitle>Kunci Periode</CardTitle>
        </CardHeader>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b">
                <th className="py-2 text-left">OPD</th>
                <th>Periode</th>
                <th>Locked</th>
                <th>Unlocked</th>
                <th>Aksi</th>
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className="border-b">
                  ...
                  <td className="text-center text-xs">
                    {r.unlocked_at ? new Date(r.unlocked_at).toLocaleString("id-ID") : "—"}
                  </td>
                  <td className="text-center">
                    {r.locked_at && !r.unlocked_at && (
                      <Button size="sm" variant="outline" onClick={() => unlock(r.id)}>
                        Buka
                      </Button>
                    )}
                  </td>
