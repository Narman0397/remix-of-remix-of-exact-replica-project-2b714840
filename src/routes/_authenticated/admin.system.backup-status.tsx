...
export const Route = createFileRoute("/_authenticated/admin/system/backup-status")({
  head: () => ({
    meta: [{ title: "Backup Status — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <SuperAdminOnly>
  ...
  return (
    <AdminShell breadcrumb={[{ label: "System" }, { label: "Backup Status" }]}>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Status snapshot terkini dan verifikasi cakupan. Tidak menggantikan backup database tingkat
        infrastruktur.
      </p>
      ...
      <div className="mb-6 grid gap-3 sm:grid-cols-3">
        <StatCard
          ...
          value={latest ? new Date(latest.created_at).toLocaleString("id-ID") : "—"}
          icon={Database}
        />
        <StatCard
          label="Usia (jam)"
          value={ageH !== null ? ageH.toFixed(1) : "—"}
          tone={ageH !== null && ageH < 48 ? "success" : "destructive"}
        />
        ...
        <ul className="space-y-2 text-sm">
          {checklist.map((c) => (
            <li key={c.item} className="flex items-center gap-2">
              <CheckCircle2
                className={`h-4 w-4 ${c.ok ? "text-success" : "text-muted-foreground"}`}
              />
              <span>{c.item}</span>
            </li>
          ))}
