...
export const Route = createFileRoute("/_authenticated/admin/desa")({
  head: () => ({
    meta: [{ title: "Master Desa — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <DesaPage />
...
function DesaPage() {
  ...
  async function save() {
    if (!editing?.nama || editing.nama.trim().length < 2) {
      toast.error("Nama desa wajib");
      return;
    }
    try {
      await upsertDesa({
        data: {
          ...
          kecamatan: editing.kecamatan?.trim() || null,
  ...
  async function hapus(id: string) {
    if (!confirm("Hapus desa? Profil warga yang sudah memilih desa ini tidak akan ikut terhapus."))
      return;
    try {
      await deleteDesa({ data: { id } });
  ...
  if (!isSuperAdmin) {
    return (
      <AdminShell breadcrumb={[{ label: "Desa" }]}>
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Daftar desa yang dipakai pada pendaftaran warga & manajemen Admin Desa.
          </p>
        </div>
        <button
          onClick={() => setEditing({ aktif: true })}
        ...
        <table className="w-full min-w-[640px] text-sm">
          ...
          <tbody>
            ...
            {!loading && rows.length === 0 && (
              <tr>
                <td colSpan={4} className="px-4 py-12 text-center text-muted-foreground">
                  Belum ada desa. Tambahkan satu untuk mulai.
                </td>
              </tr>
            )}
            {rows.map((d) => (
              <tr key={d.id} className="border-t border-border">
                ...
                <td className="px-4 py-3">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${d.aktif ? "bg-success/15 text-success" : "bg-muted text-muted-foreground"}`}
                  >
          ...
          <div className="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-elevated">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">
                {editing.id ? "Edit Desa" : "Tambah Desa"}
              </h2>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  value={editing.kecamatan ?? ""}
                  onChange={(e) => setEditing({ ...editing, kecamatan: e.target.value })}
                  className="mt-1 h-9 w-full rounded-md border border-border bg-background px-3 text-sm"
                />
              </div>
