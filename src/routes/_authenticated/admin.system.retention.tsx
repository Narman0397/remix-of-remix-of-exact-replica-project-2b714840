...
} from "@/lib/ops/retention.functions";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/admin/system/retention")({
  head: () => ({
    meta: [{ title: "Retention Policy — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <SuperAdminOnly>
...
function Page() {
  ...
  const mRun = useMutation({
    mutationFn: run,
    onSuccess: () => {
      toast.success("Retention cleanup selesai");
      qc.invalidateQueries({ queryKey: ["retention"] });
    },
    onError: (e) => toast.error((e as Error).message),
  });
      ...
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Atur berapa lama data historis disimpan sebelum dibersihkan otomatis.
          </p>
        </div>
        <button
          onClick={() => {
            if (confirm("Jalankan retention cleanup sekarang?")) mRun.mutate(undefined as never);
          }}
          ...
          <tbody>
            ...
            {(data as Policy[] | undefined)?.map((p) => (
              <PolicyRow
                ...
                p={p}
                onSave={(patch) =>
                  mUpdate.mutate({ data: { entity: p.entity, ...patch } } as never)
                }
              />
            ))}
          </tbody>
...
function PolicyRow({
  ...
  p: Policy;
  onSave: (patch: { retention_days?: number; enabled?: boolean }) => void;
}) {
    ...
    <tr className="border-t border-border">
      ...
      <td className="px-4 py-3">
        ...
        {days !== p.retention_days && (
          <button
            onClick={() => onSave({ retention_days: days })}
      ...
      <td className="px-4 py-3 text-xs text-muted-foreground">
        {p.last_run_at ? new Date(p.last_run_at).toLocaleString("id-ID") : "—"}
      </td>
