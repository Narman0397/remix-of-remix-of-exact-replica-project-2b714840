...
export const Route = createFileRoute("/_authenticated/asn/izin")({
  head: () => ({
    meta: [{ title: "Pengajuan Izin / Cuti — ASN" }, { name: "robots", content: "noindex" }],
  }),
  component: IzinPage,
});
...
function IzinPage() {
  ...
  if (!isAsn || !isVerified)
    return (
      <PageShell>
        <div className="container-page py-10">Akun ASN belum diverifikasi Super Admin.</div>
      </PageShell>
    );
  ...
  return (
    <PageShell>
      <PageHero
        ...
        description="Ajukan izin, cuti, sakit, dinas luar, atau WFH. Tanggal yang disetujui akan dikecualikan dari penghitungan alpa."
      />
      <div className="container-page py-8 grid gap-8 lg:grid-cols-2">
        <form
          onSubmit={submit}
          className="rounded-xl border border-border bg-card p-5 space-y-3 shadow-soft"
        >
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <label className="block text-sm">
              ...
              <input
                ...
                type="date"
                value={form.sampai}
                onChange={(e) => setForm({ ...form, sampai: e.target.value })}
                className="mt-1 w-full h-10 rounded-md border border-border bg-background px-3"
              />
            </label>
          </div>
          ...
          <label className="block text-sm">
            Lampiran (URL surat/sakit, opsional)
            <input
              type="url"
          ...
          <button
            disabled={busy}
            className="h-10 px-4 rounded-md bg-gradient-primary text-sm font-semibold text-primary-foreground disabled:opacity-60"
          >
        ...
        <div className="rounded-xl border border-border bg-card p-5 shadow-soft">
          ...
          <div className="mt-3 divide-y divide-border">
            {rows.length === 0 && (
              <div className="py-6 text-sm text-muted-foreground">Belum ada pengajuan.</div>
            )}
            {rows.map((r) => (
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${tone[status] ?? "bg-muted"}`}
    >
      {status.toUpperCase()}
    </span>
  );
