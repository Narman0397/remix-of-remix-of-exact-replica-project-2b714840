...
export const Route = createFileRoute("/_authenticated/admin/system/feature-flags")({
  head: () => ({
    meta: [{ title: "Feature Flags — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <SuperAdminOnly>
  ...
  return (
    <AdminShell breadcrumb={[{ label: "System" }, { label: "Feature Flags" }]}>
      ...
      <p className="mb-4 text-sm text-muted-foreground">
        Toggle fitur tanpa redeploy. Setiap perubahan tercatat di audit log.
      </p>
              ...
              return (
                <tr key={r.key} className="border-t border-border">
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${on ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}
                    >
                      {on ? "ON" : "OFF"}
                    </span>
                  </td>
