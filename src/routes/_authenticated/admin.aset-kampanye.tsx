...
export const Route = createFileRoute("/_authenticated/admin/aset-kampanye")({
  head: () => ({
    meta: [{ title: "Kampanye Verifikasi Aset" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <Page />
...
function Page() {
  ...
  const [form, setForm] = useState({
    ...
    periode_selesai: new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10),
    target_opd_ids: [] as string[],
  });
  const [progress, setProgress] = useState<
    Record<string, { total: number; verified: number; persen: number }>
  >({});

  async function reload() {
    try {
      const r = await listCampaigns();
      setRows((r as { rows: Camp[] }).rows);
  ...
  useEffect(() => {
    rows.forEach(async (c) => {
      ...
      try {
        const p = await campaignProgress({ data: { campaign_id: c.id } });
        setProgress((prev) => ({ ...prev, [c.id]: p as never }));
      } catch {
        /* noop */
      }
    });
  ...
  return (
    <AdminShell
      breadcrumb={[{ label: "Admin", to: "/admin" }, { label: "Kampanye Verifikasi Aset" }]}
    >
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-border bg-card p-4">
          ...
          <div className="mt-3 grid gap-3 md:grid-cols-3">
            <input
              ...
              placeholder="Nama (mis. Verifikasi Semester 1)"
              value={form.nama}
            ...
            <div className="max-h-40 overflow-y-auto rounded-md border border-border p-2">
              {opds.map((o) => (
                <label key={o.id} className="flex items-center gap-2 py-1 text-sm">
                  <input
                    type="checkbox"
                    checked={form.target_opd_ids.includes(o.id)}
                    onChange={(e) =>
                      setForm((f) => ({
                        ...f,
                        target_opd_ids: e.target.checked
                          ? [...f.target_opd_ids, o.id]
                          : f.target_opd_ids.filter((x) => x !== o.id),
                      }))
                    }
                  />
                  {o.singkatan} — {o.nama}
          ...
          <button
            onClick={save}
            disabled={!form.nama || form.target_opd_ids.length === 0}
            className="mt-3 h-10 rounded-md bg-gradient-primary px-4 text-sm font-semibold text-primary-foreground disabled:opacity-50"
          >
            Simpan
          </button>
        </div>
        ...
        <div className="rounded-xl border border-border bg-card">
          {rows.length === 0 && (
            <div className="p-6 text-center text-sm text-muted-foreground">Belum ada kampanye.</div>
          )}
          {rows.map((c) => {
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {c.periode_mulai} – {c.periode_selesai} · {c.target_opd_ids.length} OPD ·{" "}
                      {c.status}
                    </div>
                  </div>
                  {p && (
                    <div className="text-sm font-semibold">
                      {p.verified}/{p.total} ({p.persen}%)
                    </div>
                  )}
                </div>
                {p && (
