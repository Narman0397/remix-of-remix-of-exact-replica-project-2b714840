...
export const Route = createFileRoute("/_authenticated/admin/opd")({
  head: () => ({
    meta: [{ title: "Manajemen OPD — Admin" }, { name: "robots", content: "noindex" }],
  }),
  component: () => (
    <AdminGuard>
      <OpdPage />
...
function OpdPage() {
  ...
  async function save() {
    if (!editing?.nama || !editing?.singkatan) {
      toast.error("Nama dan singkatan wajib");
      return;
    }
    try {
      await upsertOpd({
      ...
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Kelola Organisasi Perangkat Daerah dan kategori layanannya.
          </p>
        </div>
        <button
          onClick={() => setEditing({ kategori: [] })}
          ...
          <div className="w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-elevated">
            <div className="mb-4 flex items-center justify-between">
              <h2 className="font-display text-lg font-bold">
                {editing.id ? "Edit OPD" : "Tambah OPD"}
              </h2>
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <input
                  value={editing.singkatan ?? ""}
                  onChange={(e) => setEditing({ ...editing, singkatan: e.target.value })}
                  className="mt-1 h-9 w-full rounded-md border border-border bg-background px-3 text-sm"
                />
              </div>
              <div>
                <label className="text-xs font-medium text-muted-foreground">
                  Kategori (pisah dengan koma)
                </label>
                <input
