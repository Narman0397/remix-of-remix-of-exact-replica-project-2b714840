...
function Page() {
  ...
  async function hapus(tanggal: string) {
    ...
    try {
      await deleteHariLibur({ data: { tanggal } });
  ...
  return (
    <AdminShell breadcrumb={[{ label: "Admin", to: "/admin" }, { label: "Hari Libur" }]}>
      <div className="space-y-6">
        ...
        <form
          onSubmit={submit}
          className="rounded-xl border border-border bg-card p-5 grid gap-3 md:grid-cols-4"
        >
          ...
          <label className="text-sm flex items-center gap-2 mt-6">
            <input
              type="checkbox"
              checked={form.nasional}
              onChange={(e) => setForm({ ...form, nasional: e.target.checked })}
            />
            Nasional
          </label>
          ...
          <button className="h-10 rounded-md bg-gradient-primary px-4 text-sm font-semibold text-primary-foreground mt-6">
            Simpan
          </button>
        </form>
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-4 py-6 text-center text-muted-foreground">
                    Belum ada hari libur tahun {year}.
                  </td>
                </tr>
              )}
              {rows.map((r) => (
