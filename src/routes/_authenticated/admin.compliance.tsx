...
function Page() {
  ...
  const [summary, setSummary] = useState<
    Record<string, { total: number; done: number; in_progress: number; todo: number; na: number }>
  >({});

  async function load() {
    try {
      const [rows, sum] = await Promise.all([
        fnList({}) as Promise<Item[]>,
        fnSum({}) as Promise<typeof summary>,
      ]);
      setItems(rows);
      setSummary(sum);
    } catch (e) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground">
          SPBE, SMKI, perlindungan data, & interoperabilitas.
        </p>
      </div>
            ...
            <Card key={d}>
              ...
              <CardContent>
                ...
                <div className="text-[10px] text-muted-foreground">
                  {s.done}/{s.total} selesai · {s.in_progress} proses
                </div>
              </CardContent>
            </Card>
          );
                  ...
                  .map((i) => (
                    <tr key={i.id}>
                      ...
                      <td className="px-3 py-2">
                        ...
                        {i.deskripsi && (
                          <div className="text-xs text-muted-foreground">{i.deskripsi}</div>
                        )}
                      </td>
                      <td className="px-3 py-2">
                        <span
                          className={`rounded px-1.5 py-0.5 text-[10px] font-semibold uppercase ${STATUS_COLOR[i.status]}`}
                        >
                      ...
                      <td className="px-3 py-2">
                        <div className="flex flex-wrap gap-1">
                          ...
                          <Button size="sm" variant="ghost" onClick={() => setStatus(i.id, "na")}>
                            N/A
                          </Button>
                          <Button size="sm" variant="ghost" onClick={() => setCatatan(i.id)}>
                            Catatan
                          </Button>
                        </div>
                      </td>
