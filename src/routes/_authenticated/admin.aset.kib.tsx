...
function Page() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [a, s] = await Promise.all([listAset({ data: {} }), kibSummary({ data: {} })]);
    ...
    <div className="space-y-4">
      <div>
        <h2 className="font-display text-xl font-bold flex items-center gap-2">
          <Layers className="h-5 w-5" /> KIB Aset
        </h2>
        <p className="text-sm text-muted-foreground">
          Klasifikasi Kartu Inventaris Barang sesuai Permendagri.
        </p>
      </div>
        ...
        <CardHeader>
          <CardTitle className="text-sm flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <select
                value={kib}
                onChange={(e) => setKib(e.target.value as (typeof KIB)[number])}
              ...
              <Button size="sm" onClick={apply} disabled={selected.size === 0}>
                <Save className="h-3.5 w-3.5 mr-1" /> Terapkan
              </Button>
            </div>
          </CardTitle>
        </CardHeader>
                ...
                <tbody>
                  {rows.map((r) => (
                    <tr key={r.id} className="border-b hover:bg-muted/30">
                      ...
                      <td className="p-2">
                        <Badge variant={r.kib ? "default" : "outline"}>{r.kib ?? "-"}</Badge>
                      </td>
                    </tr>
                  ))}
                </tbody>
