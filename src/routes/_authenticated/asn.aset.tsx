...
export const Route = createFileRoute("/_authenticated/asn/aset")({
  head: () => ({
    meta: [{ title: "Tracking Aset — Scan QR" }, { name: "robots", content: "noindex" }],
  }),
  component: AsetPage,
});
...
function AsetPage() {
  ...
  const requestGps = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      (err) => {
        ...
        setGpsError(
          err.code === 1
            ? "Izin lokasi ditolak. Aktifkan GPS pada peramban Anda."
  ...
  const reloadMine = useCallback(async () => {
    try {
      ...
      setMine((r as unknown as { rows: Aset[] }).rows);
    } catch (e) {
      console.warn("[aset]", (e as Error).message);
    }
  }, []);
  useEffect(() => {
    if (user) reloadMine();
  ...
  const onScan = useCallback(async (text: string) => {
    ...
    try {
      ...
      setScanned(a as unknown as Aset);
  ...
  async function submitScan() {
    ...
    if (!deadline || Date.now() > deadline) {
      toast.error("Waktu 60 detik habis. Scan ulang QR aset.");
      return;
    }
    if (!photoBlob) {
      toast.error("Wajib mengambil foto fisik aset dari kamera.");
      return;
    }
    ...
    <PageShell>
      <section className="container-page py-8">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Scan QR aset → ambil foto fisik dalam 60 detik (hanya kamera langsung) → kirim.
        </p>
          ...
          <div>
            {!gpsReady ? (
              ...
              <div className="rounded-xl border border-border bg-card p-4 space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">
                      {scanned.kode} · {scanned.kategori} · {scanned.status}
                    </div>
                    <div className="text-xs text-muted-foreground">
                      OPD: {scanned.opd?.nama ?? "-"}
                    </div>
                  </div>
                  <button onClick={resetCapture} className="rounded-md border border-border p-1.5">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                ...
                {/* Countdown */}
                <div
                  className={`rounded-md p-3 text-sm ${expired ? "bg-destructive/15 text-destructive" : remainingSec <= 15 ? "bg-gold/20" : "bg-primary-soft text-primary"}`}
                >
                  {expired ? (
                    <div>
                      <div className="font-semibold">Waktu habis.</div>
                      <p className="text-xs">
                        Foto fisik aset harus diambil dalam 60 detik setelah QR diverifikasi.
                        Silakan scan ulang QR.
                      </p>
                    </div>
                  ) : (
                    ...
                    {!photoPreview ? (
                      ...
                      <div className="space-y-2">
                        <img
                          src={photoPreview}
                          alt="Foto aset"
                          className="w-full max-h-64 rounded-md object-contain border border-border bg-black/5"
                        />
                        <button
                          onClick={() => {
                            setPhotoBlob(null);
                ...
                {expired && (
                  <button
                    onClick={resetCapture}
                    className="h-10 w-full rounded-md bg-gradient-primary text-sm font-semibold text-primary-foreground"
                  >
                    Scan Ulang QR
                  </button>
                )}
            ...
            <div className="mt-2 space-y-2">
              ...
              {mine.map((a) => (
                <div key={a.id} className="rounded-md border border-border bg-card p-3 text-sm">
                  ...
                  <div className="text-xs text-muted-foreground">
                    {a.kode} · {a.kategori} · {a.status}
                  </div>
                  {a.lokasi_terkini && (
                    <div className="text-xs text-muted-foreground">📍 {a.lokasi_terkini}</div>
                  )}
                </div>
              ))}
            </div>
