...
export const listIzinSelf = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    const { data, error } = await supabaseAdmin
      .from("pengajuan_izin")
      .select(
        "id,jenis,dari,sampai,alasan,lampiran_url,status,catatan_approval,approved_at,created_at",
      )
...
export const listIzinAdmin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = supabaseAdmin
      .from("pengajuan_izin")
      .select(
        "id,user_id,opd_id,jenis,dari,sampai,alasan,lampiran_url,status,catatan_approval,created_at",
      )
      .order("created_at", { ascending: false })
      .limit(300);
    ...
    const [{ data: profs }, { data: opds }] = await Promise.all([
      userIds.length
        ...
        : Promise.resolve({
            data: [] as { id: string; nama_lengkap: string | null; nip: string | null }[],
          }),
      opdIds.length
    ...
    const profMap = new Map(
      (profs ?? []).map((p) => [p.id, { nama_lengkap: p.nama_lengkap, nip: p.nip }]),
    );
    const opdMap = new Map(
      (opds ?? []).map((o) => [o.id, { nama: o.nama, singkatan: o.singkatan }]),
    );
    return {
      rows: list.map((r) => ({
        ...r,
        profile: profMap.get(r.user_id) ?? null,
        opd: r.opd_id ? (opdMap.get(r.opd_id) ?? null) : null,
      })),
    };
  });
...
export const decideIzin = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (data.status === "rejected" && !data.catatan)
      throw new Error("Wajib mengisi catatan saat menolak");
...
export const attendanceDeviceAlert = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const c = await ctxOf(context.userId);
    if (!c.isSuper) throw new Error("Hanya super admin");
    const { data: rows, error } = await supabaseAdmin.rpc("attendance_device_alert", {
      _days: data.days,
    });
    if (error) throw new Error(error.message);
    return { rows: rows ?? [] };
