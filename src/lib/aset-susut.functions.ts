...
export const runSusutBulanan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: res, error } = await supabaseAdmin.rpc("fn_susut_bulanan_run", {
    ...
    await supabaseAdmin.from("audit_log").insert({
      ...
      aksi: "aset.susut_run",
      entitas: "aset_penyusutan_history",
      entitas_id: data.periode,
      data_sesudah: res as never,
    });
    return res;
  });
...
export const listSusutHistory = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const c = await getUserContext(supabaseAdmin, context.userId);
    let q = supabaseAdmin
      .from("aset_penyusutan_history")
      .select(
        "id,aset_id,periode,susut_bulan,akumulasi,nilai_buku,created_at, aset:aset!aset_id(kode,nama,opd_id,pemegang_user_id)",
      )
    ...
    const filtered = (rows ?? []).filter((r) => {
      ...
      const a = (
        r as unknown as { aset: { opd_id: string | null; pemegang_user_id: string | null } | null }
      ).aset;
      if (!a) return false;
