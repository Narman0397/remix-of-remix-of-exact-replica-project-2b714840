...
export const listOpname = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    let q = supabaseAdmin
      .from("aset_opname")
      .select(
        "id,opd_id,periode,status,catatan,closed_at,created_at, opd:opd!opd_id(singkatan,nama)",
      )
...
export const listOpnameItems = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: o } = await supabaseAdmin
      .from("aset_opname")
      .select("opd_id")
      .eq("id", data.opname_id)
      .maybeSingle();
    if (!o) throw new Error("Tidak ditemukan");
    if (!c.isSuper && !(c.isAdminOpd && c.opdId === o.opd_id)) throw new Error("Forbidden");
    const { data: rows, error } = await supabaseAdmin
      .from("aset_opname_items")
      .select(
        "id,hadir,kondisi_temuan,catatan,verified_at, aset:aset!aset_id(id,kode,nama,kategori,lokasi_terkini)",
      )
      .eq("opname_id", data.opname_id)
      .limit(2000);
    if (error) throw new Error(error.message);
    return { rows: rows ?? [] };
  });
...
export const verifyOpnameItem = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: it } = await supabaseAdmin
      .from("aset_opname_items")
      .select("opname_id, opname:aset_opname!opname_id(opd_id,status)")
      .eq("id", data.item_id)
      .maybeSingle();
    if (!it) throw new Error("Tidak ditemukan");
    const opname = (it as unknown as { opname: { opd_id: string | null; status: string } | null })
      .opname;
    ...
    const { error } = await supabaseAdmin
      ...
      .update({
        hadir: data.hadir,
        kondisi_temuan: data.kondisi_temuan ?? null,
        catatan: data.catatan ?? null,
