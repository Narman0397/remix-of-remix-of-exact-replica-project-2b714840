...
export const listGedung = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    let q = supabaseAdmin
      .from("lokasi_gedung")
      .select("id,nama,alamat,opd_id, opd:opd!opd_id(singkatan)")
      .order("nama");
...
export const upsertRuangan = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: l } = await supabaseAdmin
      .from("lokasi_lantai")
      .select("gedung_id, gedung:lokasi_gedung!gedung_id(opd_id)")
      .eq("id", data.lantai_id)
      .maybeSingle();
    const opdId =
      (l as { gedung: { opd_id: string | null } | null } | null)?.gedung?.opd_id ?? null;
...
export const listHierarki = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: lantais } = ids.length
      ? await supabaseAdmin
          .from("lokasi_lantai")
          .select("id,gedung_id,nama,urutan")
          .in("gedung_id", ids)
          .order("urutan")
    ...
    const { data: ruangans } = lids.length
      ? await supabaseAdmin
          .from("lokasi_ruangan")
          .select("id,lantai_id,nama,kode,pic_user_id")
          .in("lantai_id", lids)
          .order("nama")
      : {
          data: [] as Array<{
