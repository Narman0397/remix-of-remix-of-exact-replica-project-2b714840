...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  async function loadPermissions(uid: string, attempt = 0): Promise<void> {
    ...
    if (error) {
      ...
      // Beri sinyal lembut ke UI tanpa memblok render.
  ...
  // sejak terakhir fetch) — backstop bila koneksi realtime sempat putus, agar
  // status verifikasi tersinkron antara dashboard admin dan akun ASN.
  useEffect(() => {
    if (!user?.id) return;
