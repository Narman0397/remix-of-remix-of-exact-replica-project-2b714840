...
export const createBast = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: asets } = await supabaseAdmin
      ...
      .select("id,opd_id")
      .in("id", data.aset_ids);
    if (!c.isSuper && (asets ?? []).some((a) => a.opd_id !== c.opdId))
      throw new Error("Ada aset di luar OPD Anda");
    const nomor = genNomor();
    const { data: row, error } = await supabaseAdmin
      .from("aset_bast")
      .insert({
        nomor,
        pemberi_user: context.userId,
        penerima_user: data.penerima_user,
        opd_id: opdId,
...
export const cancelBast = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((i: unknown) =>
    z.object({ id: z.string().uuid(), alasan: z.string().max(500).optional() }).parse(i),
  )
  .handler(async ({ data, context }) => {
    const c = await getUserContext(supabaseAdmin, context.userId);
    const { data: b } = await supabaseAdmin
      .from("aset_bast")
      .select("opd_id,status,created_by")
      .eq("id", data.id)
      .maybeSingle();
    if (!b) throw new Error("BAST tidak ditemukan");
    if (b.status === "approved") throw new Error("BAST sudah disetujui");
    if (!c.isSuper && !(c.isAdminOpd && c.opdId === b.opd_id) && b.created_by !== context.userId)
      throw new Error("Forbidden");
    const { error } = await supabaseAdmin
      .from("aset_bast")
      .update({ status: "cancelled", catatan: data.alasan ?? null } as never)
      .eq("id", data.id);
    if (error) throw new Error(error.message);
    return { ok: true };
  });
...
export const listBast = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = supabaseAdmin
      .from("aset_bast")
      .select(
        "id,nomor,tanggal,status,catatan,pemberi_user,penerima_user,opd_id,approved_at,created_at, pemberi:profiles!pemberi_user(nama_lengkap), penerima:profiles!penerima_user(nama_lengkap), opd:opd!opd_id(singkatan,nama)",
      )
      .order("created_at", { ascending: false })
      .limit(200);
    if (data.status) q = q.eq("status", data.status);
    if (!c.isSuper) {
      if (c.isAdminOpd && c.opdId)
        q = q.or(
          `opd_id.eq.${c.opdId},pemberi_user.eq.${context.userId},penerima_user.eq.${context.userId}`,
        );
...
export const getBastDetail = createServerFn({ method: "POST" })
  ...
  .inputValidator((i: unknown) => z.object({ id: z.string().uuid() }).parse(i))
  .handler(async ({ data }) => {
    const { data: bast, error } = await supabaseAdmin
      .from("aset_bast")
      .select(
        "*, pemberi:profiles!pemberi_user(nama_lengkap,nip), penerima:profiles!penerima_user(nama_lengkap,nip), opd:opd!opd_id(nama,singkatan)",
      )
      .eq("id", data.id)
      .maybeSingle();
    if (error) throw new Error(error.message);
