...
function ItemsTab({ kategori }: { kategori: Kategori }) {
  ...
  async function save() {
    if (!editing?.label) {
      toast.error("Label/judul wajib diisi");
      return;
    }
    const payload = {
      kategori,
    ...
    <>
      <div className="mb-3 flex items-center justify-between">
        <div className="text-sm text-muted-foreground">
          {KATEGORI_LABEL[kategori]} — {rows.length} item
        </div>
        <button
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-border">
                ...
                <td className="px-4 py-3 text-muted-foreground">
                  {r.nilai_teks ??
                    (r.nilai_num !== null
                      ? `${r.nilai_num}${r.nilai_num2 !== null ? ` / ${r.nilai_num2}` : ""}`
                      : "—")}
                ...
                {kategori === "dataset" && (
                  <td className="px-4 py-3 text-xs text-muted-foreground">
                    {r.opd ?? "—"}{" "}
                    {r.format && (
                      <span className="ml-1 rounded bg-primary-soft px-1.5 py-0.5 text-primary">
                        {r.format}
                      </span>
                    )}
                  </td>
                ...
                <td className="px-4 py-3">
                  ...
                  >
                    <span
                      className={`inline-block h-4 w-4 rounded-full bg-background shadow transition-transform ${r.aktif ? "translate-x-5" : "translate-x-0.5"}`}
                    />
                  </button>
                </td>
  ...
  return (
    <Modal title={title} onClose={onClose} onSave={onSave}>
      <Input
        label={
          kategori === "chart_layanan"
            ? "Label periode (contoh: Jan)"
            : kategori === "penduduk"
              ? "Kelompok (contoh: 18-35)"
              : kategori === "anggaran"
                ? "Sektor (contoh: Pendidikan)"
                : kategori === "dataset"
                  ? "Judul dataset"
                  : "Label KPI"
        }
      ...
      {kategori === "kpi" && (
        <>
          <Input
            label="Nilai (teks, contoh: 1.42 Juta atau 67.8%)"
            value={editing.nilai_teks ?? ""}
            onChange={(v) => setEditing({ ...editing, nilai_teks: v })}
          />
          <Input
            label="Tren / keterangan kecil"
            value={editing.trend ?? ""}
            onChange={(v) => setEditing({ ...editing, trend: v })}
            placeholder="+1.2% YoY"
          />
          <div>
            ...
            <select
              value={editing.ikon ?? ""}
              onChange={(e) => setEditing({ ...editing, ikon: e.target.value || null })}
      ...
      {kategori === "chart_layanan" && (
        <div className="grid gap-3 md:grid-cols-2">
          <NumberInput
            ...
            value={editing.nilai_num}
            onChange={(v) => setEditing({ ...editing, nilai_num: v })}
          />
          <NumberInput
            label="Permohonan selesai"
            value={editing.nilai_num2}
            onChange={(v) => setEditing({ ...editing, nilai_num2: v })}
          />
        </div>
      )}
        ...
        <>
          <div className="grid gap-3 md:grid-cols-2">
            <Input
              label="OPD penyedia"
              value={editing.opd ?? ""}
              onChange={(v) => setEditing({ ...editing, opd: v })}
            />
            <Input
              label="Format (CSV, XLSX, JSON, GeoJSON)"
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <Input
              label="Ukuran (mis. 2.4 MB)"
              value={editing.ukuran ?? ""}
              onChange={(v) => setEditing({ ...editing, ukuran: v })}
            />
            <Input
              label="URL unduh"
              value={editing.url ?? ""}
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <NumberInput
          ...
          onChange={(v) => setEditing({ ...editing, urutan: v ?? 0 })}
        />
