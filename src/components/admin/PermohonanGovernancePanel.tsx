...
export function PermohonanGovernancePanel({
  ...
  async function load() {
    ...
    setSla((a.events ?? []) as unknown as SlaEvent[]);
    setDisp((b.rows ?? []) as unknown as Disposisi[]);
  }
  ...
  async function doDispose() {
    ...
    try {
      await fnDispose({
        data: { permohonan_id: permohonanId, to_user: to, level, note: note || undefined },
      });
    ...
    <div className="space-y-4">
        ...
        <ol className="mb-3 space-y-2 text-sm">
          ...
          {disp.map((d) => (
            <li key={d.id} className="rounded-md border border-border p-2">
              ...
              <div className="text-xs text-muted-foreground">
                Kepada: {profileMap.get(d.to_user) ?? d.to_user.slice(0, 8)} ·{" "}
      ...
      <div className="rounded-xl border border-border bg-card p-5 shadow-soft">
        ...
        <div className="space-y-2 text-sm">
          <div>
            Nomor Surat: <span className="font-mono font-medium">{nomor ?? "—"}</span>
          </div>
          {!nomor && (
            <button
              onClick={doIssue}
          ...
          >
            {busy ? (
              ...
              "Terbitkan Ulang Dokumen"
            ) : (
              "Terbitkan Dokumen Final (PDF)"
            )}
          </button>
          {docPath && (
            <button
              onClick={openDoc}
              className="inline-flex w-full items-center justify-center gap-2 rounded-md border border-border px-3 py-2 text-sm font-medium hover:bg-muted"
            >
